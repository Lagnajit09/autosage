...
class PowerShellExecutor:
    ...
    @staticmethod
    def _build_env_block(env_vars: dict) -> str:
        """
        Build a PowerShell block that sets session-scoped environment variables.

        Variables are assigned using single-quoted strings.  Embedded single
        quotes are doubled ('' is the PowerShell escape for a literal quote
        inside a single-quoted string) to prevent injection.

        Variable names are validated against the POSIX identifier pattern so
        that malformed names cannot break the PowerShell syntax.
        """
        lines = [
    ...
    def _build_ps_invocation(self, script_content: str, env_vars: Optional[dict] = None) -> str:
        """
        Wrap the user's PowerShell script so that:
          1. All workflow input parameters are available as $env: variables.
          2. The execution policy is bypassed for this session.
          3. Errors are written to stderr (via $ErrorActionPreference).
          4. We get a clean exit code.

        The entire script (env block + user code) is Base64-encoded before
        being passed to PowerShell, so secret values are never visible in
        process listings or WinRM command arguments.
        """
        import base64
        # Prepend env var assignments if inputs were provided
        env_block = self._build_env_block(env_vars) if env_vars else ""
        full_script = env_block + script_content
    ...
    async def stream(
        ...
        try:
            ...
            # Build PowerShell invocation command (env vars embedded + Base64-encoded)
            ps_cmd = self._build_ps_invocation(script_content, env_vars=env_vars)
            if env_vars:
                secret_count = len(secret_keys) if secret_keys else 0
                logger.info(
                    "Injecting %d env vars (%d secret) into PowerShell session on %s",
